...
import com.canerture.satellitesapp.common.fromJson
...
class JsonDataSourceImpl(
    ...
    override suspend fun getSatellites(): Resource<List<Satellite>> =
        try {
            Resource.Success(
                context.fromJson<Array<Satellite>>(Json.SATELLITE_LIST_JSON).toList()
            )
        } catch (e: Exception) {
    ...
    override suspend fun getSatelliteDetail(satelliteId: Int): Resource<SatelliteDetail?> =
        try {
            Resource.Success(
                context.fromJson<Array<SatelliteDetail>>(Json.SATELLITE_DETAIL_JSON).toList()
                    .find {
                        it.id == satelliteId
                    }
            )
    ...
    override suspend fun getSatellitePositions(satelliteId: Int): Resource<SatellitePosition?> =
        try {
            Resource.Success(
                context.fromJson<SatellitePositionsList>(Json.SATELLITE_POSITIONS_JSON).list.find {
                    it.id == satelliteId
                }
            )
        } catch (e: Exception) {
